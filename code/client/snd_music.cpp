...
static qboolean Music_ParseMusic( gsl::czstring filename, const CGenericParser2& Parser, MusicData_t* MusicData, const CGPGroup& pgMusicFiles, const std::string_view& psMusicName, const std::string_view& psMusicNameKey, MusicState_e eMusicState )
{
	bool bReturn = false;
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				//
				int steps = 0;
				std::string_view searchName{ &sLevelName[ 0 ], &sLevelName[ strlen( &sLevelName[ 0 ] ) ] };
				...
				else
				{
					...
					//
					std::string_view psName_Explore;
					std::string_view psName_Action;
					std::string_view psName_Boss;
					std::string_view psName_UseBoss;
