...
{
public:
	using Values = GP2::Vector<std::string_view>;
private:
...
public:
	...
	CGPProperty( std::string_view initKey, std::string_view initValue = {} );

	const std::string_view& GetName() const { return mKey; }
	bool IsList() const NOEXCEPT
	{
		return mValues.size() > 1;
	}
	const std::string_view& GetTopValue() const NOEXCEPT
	{
	...
	void AddValue( std::string_view newValue );
};
...
private:
	...
	std::string_view mName = CSTRING_VIEW( "Top Level" );
	SubGroups mSubGroups;
public:
	CGPGroup() = default;
	CGPGroup( const std::string_view& initName );
	...
	const CGPGroup* FindSubGroup( const std::string_view& name ) const NOEXCEPT
	{
	...
	const CGPProperty* FindProperty( const std::string_view& name ) const NOEXCEPT
	{
	...
	bool Parse( std::string_view& data, const bool topLevel = true );
};
